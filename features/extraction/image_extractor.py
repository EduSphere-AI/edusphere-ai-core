    ...
    for img_idx, img in enumerate(image_list):
        ...
        for rect in image_rects:
            ...
            image_data = {
                ...
                "bpc":
                base_image.get("bpc", 8),  # bits per component
                "ext":
                base_image["ext"],
                "size":
                base_image["image"].__sizeof__()
                if "image" in base_image else 0
            }
            ...
            if overlapping_text:
                image_data["embedded_text"] = [
                    t["text"] for t in overlapping_text
                ]
            else:
                image_data["embedded_text"] = []
...
def _find_overlapping_text(page: fitz.Page,
                           image_bbox: Dict[str, float]) -> List[Dict]:
    """
...
def _find_image_label(page: fitz.Page,
                      image_bbox: Dict[str, float]) -> Optional[Dict]:
    """
    ...
    figure_patterns = [
        ...
        r'[Ii]mage\s+\d+', r'[Pp]hoto\s+\d+', r'[Pp]icture\s+\d+'
    ]
    ...
    for search_region in search_regions:
        for block in text_blocks:
            ...
            if (search_region["x0"] <= block_center_x <= search_region["x1"]
                    and search_region["y0"] <= block_center_y <=
                    search_region["y1"]):
