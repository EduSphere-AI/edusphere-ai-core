...
def extract_chart_components(page: fitz.Page,
                             chart_region: Dict[str, float]) -> Dict:
    """
    ...
    for block in text_blocks:
        ...
        elif (abs(block_center_x - chart_region["x0"]) < 30
              or abs(block_center_x - chart_region["x1"]) < 30
              or abs(block_center_y - chart_region["y0"]) < 30
              or abs(block_center_y - chart_region["y1"]) < 30):
    ...
    for text in chart_texts:
        ...
        if font_size > 10 and (text_center_y < chart_center_y - 20
                               or abs(text_center_x - chart_center_x) < 30):
            if title is None or font_size > title["font"]["size"]:
                title = text

        # Axis labels: near edges
        elif (abs(text_center_x - chart_region["x0"]) < 20
              or abs(text_center_x - chart_region["x1"]) < 20
              or abs(text_center_y - chart_region["y0"]) < 20
              or abs(text_center_y - chart_region["y1"]) < 20):
            axis_labels.append(text)

        # Data labels: small text, near center
        elif font_size < 9 and abs(
                text_center_x - chart_center_x) < chart_region["width"] * 0.4:
...
def find_chart_caption(page: fitz.Page,
                       chart_region: Dict[str, float]) -> Optional[Dict]:
    """
    ...
    figure_patterns = [
        r'[Ff]igure\s+\d+', r'[Ff]ig\.\s*\d+', r'[Ff]ig\s+\d+',
        r'[Gg]raph\s+\d+', r'[Cc]hart\s+\d+'
    ]
            ...
            for pattern in figure_patterns:
                if re.search(pattern, text):
                    ...
                    return {
                        "caption_text": text,
                        "figure_id":
                        f"figure_{figure_num}" if figure_num else None,
