        ...
        if drawing["type"] == "l":  # line
            ...
            for item in items:
                if item[0] == "l":  # line item
                    ...
                    if abs(y1 - y0) < 3 and abs(x1 -
                                                x0) > 10:  # horizontal line
                        horizontal_lines.append((min(x0, x1), max(x0, x1), y0))
                    elif abs(x1 - x0) < 3 and abs(y1 -
                                                  y0) > 10:  # vertical line
    ...
    for h_cluster in h_clusters:
        for v_cluster in v_clusters:
            ...
            if width > 50 and height > 50:
                ...
                h_lines_in_region = [
                    l for l in horizontal_lines if h_min_x <= l[0] <= h_max_x
                    and v_min_y <= l[2] <= v_max_y
                ]
                v_lines_in_region = [
                    l for l in vertical_lines if v_min_y <= l[0] <= v_max_y
                    and h_min_x <= l[2] <= h_max_x
                ]
...
def _cluster_lines(lines: List[Tuple], axis: int,
                   threshold: float) -> List[List[Tuple]]:
    """
...
def reconstruct_table_structure(
        page: fitz.Page, table_region: Dict[str, float]) -> Optional[Dict]:
    """
    ...
    table_texts = []
    for block in text_blocks:
        if (table_region["x0"] <= block["bbox"]["x0"] <= table_region["x1"]
                and table_region["y0"] <= block["bbox"]["y0"] <=
                table_region["y1"]):
    ...
    for y_cluster in sorted(y_clusters):
        ...
        row_texts = [
            t for t in table_texts if abs(t["bbox"]["y0"] - row_y) < 5
        ]
...
def _cluster_positions(positions: List[float],
...
def validate_table(
        ...
        table_region: Dict[str, float]) -> Tuple[bool, Dict[str, any]]:
    """
    ...
    if len(set(col_counts)) == 1 and col_counts[0] == num_cols:
        ...
        consistency_ratio = col_counts.count(most_common_cols) / len(
            col_counts)
    ...
    is_valid = (validation_info["has_consistent_columns"]
                and validation_info["has_multiple_rows"]
                and validation_info["column_alignment_score"] > 0.5)
