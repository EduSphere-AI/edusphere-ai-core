...
class TextCategorizer:
    ...
    def extract_features(self, text_block: Dict, page_width: float,
        ...
        features = [
            ...
            bbox["y0"] /
            page_height,  # Relative vertical position (0 = top, 1 = bottom)
            ...
            1.0 if text_block["text"].strip().startswith('"') else
            0.0,  # Starts with quote
            1.0 if text_block["text"].strip().endswith('"') else
            0.0,  # Ends with quote
    ...
    def train(self, text_blocks: List[Dict], labels: List[str],
        ...
        # Extract features
        X = np.array([
            self.extract_features(block, page_width, page_height)
    ...
    def predict(self, text_block: Dict, page_width: float,
        ...
        if self.is_trained and self.classifier:
            features = np.array(
                [self.extract_features(text_block, page_width, page_height)])
            features_scaled = self.scaler.transform(features)
            return self.classifier.predict(features_scaled)[0]
        else:
            # Fall back to rule-based classification
            return self._rule_based_classify(text_block, page_width,
                                             page_height)

    def _rule_based_classify(self, text_block: Dict, page_width: float,
                             page_height: float) -> str:
        """
...
def filter_text_by_exclusions(
        text_blocks: List[Dict], table_regions: List[Dict[str, float]],
...
def extract_text(
        ...
        categorizer: Optional[TextCategorizer] = None
) -> Dict[str, List[Dict]]:
    """
    ...
    filtered_text = filter_text_by_exclusions(all_text_blocks, table_regions,
                                              chart_regions, image_regions)

    if not filtered_text:
        return {"titles": [], "subtitles": [], "body": [], "citations": []}
    ...
    # Categorize text
    categorized = {"titles": [], "subtitles": [], "body": [], "citations": []}
